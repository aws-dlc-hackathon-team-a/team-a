...
| **ActionTicket**   | チケット生成・完了申告・Done時Persona_Message生成             | ActionTicketLambda、ActionLogDB、UserDB、BedrockClient                |
...
| **Batch**          | 日次バッチ（Action_Ticket自動破棄・DailySummary集計・破棄メッセージBedrock生成）・週次バッチ（行動モデル構築・Future_Self_Model更新）| DailyAggregationLambda、LearningEngineLambda、EventBridgeScheduler、BedrockClient   |
...
4. **Done申告フロー**: Ticket Done → Action_Log記録（リアルタイム）→ Effort_Point付与 → Bedrock で Persona_Message 生成 → レスポンスに含めて返却 → 画面表示
...
10. **学習データリセットフロー（FR-11-3）**: ProfileScreens リセットボタン → 確認ダイアログ → UserLambda で UserDB（BehaviorModel・FutureSelfModel）リセット + ActionLogDB（ActionLogEntry）全削除
...
UserLambda ─────────────────────────────────→ UserDB
                                            → ActionLogDB（学習データリセット時の ActionLogEntry 全削除）
                                            → Bedrock（AIサジェスト・Pivot_Goal生成）

ActionTicketLambda ─────────────────────────→ ActionLogDB（ActionLogEntry・EffortPointRecord書き込み、UserStatsアトミック更新）
                                            → UserDB（Done申告時のPersona_Message生成用にProfile・FutureSelfModel参照）
                                            → Bedrock（Done申告時のPersona_Message生成）
...
DailyAggregationLambda ─────────────────────→ ActionLogDB（expireTickets・DailySummary集計・discardMessage書き込み）
                                            → UserDB（Profile・FutureSelfModel 取得）
                                            → Bedrock（破棄メッセージ生成）
...
専用BedrockServiceを作らず、各Lambda（UserLambda・RecommendationLambda・ActionTicketLambda・DailyAggregationLambda・LearningEngineLambda）から直接AWS SDK v3でBedrockを呼び出す。共通処理はBackendErrorHandlerのBedrockエラーハンドリングで対応する。
...
`StatsLambda` が扱う集計は粒度ごとに実装を分ける:

- **累計（`getTotalPoints`）**: ActionLogDB の `UserStats` アイテムを 1 GetItem で取得する。`UserStats.totalPoints` は `ActionTicketLambda.completeTicket` 時に DynamoDB `UpdateItem` の `ADD` でアトミック更新されるため、読み取りは常に最新値となる。期間に比例したスキャンが不要で、HomeScreen マウント時の頻繁な取得に耐える
- **日次（`getDailySummary`）**: 過去日は `DailyAggregationLambda` が生成済みの `DailySummary` を 1 GetItem で取得する。当日分のみ `EffortPointRecord` を Query して集計する
- **週次・月次（`getWeeklySummary`/`getMonthlySummary`）**: `DailySummary` を該当期間分 Query し、StatsLambda 内で合算する。別テーブルや週次/月次バッチは用意しない

マイルストーン判定（`checkMilestone`）は `UserStats.lastMilestoneValue` を参照して重複通知を防ぐ純粋関数として実装する（PBT対象）。
