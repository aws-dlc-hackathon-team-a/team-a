...
| 画面ID | 画面名                   | 概要                                           |
| ------ | ------------------------ | ---------------------------------------------- |
| S01    | サインアップ画面         | メール・パスワード入力で新規登録               |
| S02    | メール確認画面           | 確認コード入力・再送信                         |
| S03    | ログイン画面             | メール・パスワードでログイン                   |
| S04    | パスワードリセット画面   | リセットコード送信・新パスワード設定           |
| S05    | プロフィール登録画面     | 氏名・年齢・職業・興味・生活リズム・悩みを入力 |
| S06    | Goal設定画面             | Goal登録・Primary_Goal指定                     |
| S07    | ホーム画面               | 手動TriggerボタンとOpen Ticket一覧へのアクセス |
| S08    | 心理状態入力画面         | 手動Trigger時の任意入力（スキップ可）          |
| S09    | Recommendation画面       | 提案表示・4択応答                              |
| S10    | Action_Ticket一覧画面    | Open/Done/Discarded一覧・完了申告              |
| S11    | Effort_Pointサマリー画面 | 1日の終わりの集計・Persona_Message表示         |
| S12    | ポイント履歴画面         | 週間・月間グラフ・累計ポイント                 |
| S13    | マイルストーン画面       | バッジ・達成メッセージ表示                     |
| S14    | プロフィール編集画面     | Profile情報の編集・更新履歴閲覧                |
| S15    | Goal管理画面             | Goal追加・削除・Primary_Goal変更               |
| S16    | 設定画面                 | Trigger設定・集計時刻・学習データリセット      |
| S17    | アカウント削除画面       | 削除確認ダイアログ・実行                       |
...
stateDiagram-v2
    [*] --> S03 : 初回起動（未ログイン）
    [*] --> S07 : 初回起動（ログイン済み・オンボーディング完了）

    %% 認証フロー
    S03 --> S01 : 新規登録へ
    S03 --> S04 : パスワードを忘れた
    S03 --> S05 : ログイン成功（初回）
    S03 --> S07 : ログイン成功（オンボーディング完了済み）
    S01 --> S02 : 登録完了
    S02 --> S05 : メール確認完了
    S02 --> S02 : 確認コード再送信
    S04 --> S03 : パスワードリセット完了
    ...
    %% メインフロー（自動Trigger）
    S07 --> S09 : アプリ起動時にTrigger発火
    S07 --> S08 : 手動Triggerボタンをタップ
    S08 --> S09 : 心理状態入力完了 or スキップ

    %% Recommendationフロー
    S09 --> S10 : 4択応答（やる/別の方法/目標チェンジ/自由入力）
    S09 --> S09 : 「いいえ（別の方法で）」→ 同Goal内別提案
    S09 --> S09 : 「目標チェンジ」→ Pivot提案
    S10 --> S07 : ホームへ戻る

    %% 1日の終わり
    S07 --> S11 : 集計時刻到達（アプリ内通知）
    S11 --> S12 : ポイント履歴を見る
    S12 --> S13 : マイルストーン達成時
    S11 --> S07 : ホームへ戻る
