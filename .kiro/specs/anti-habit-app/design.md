...
| S07    | ホーム画面                   | 画面     | 手動TriggerボタンとOpen Ticket一覧へのアクセス |
| S08    | 心理状態入力モーダル         | モーダル | 手動Trigger時の任意入力（スキップ可）          |
| S09    | Recommendation画面           | 画面     | 提案表示・4択応答                              |
| S10    | Action_Ticket一覧画面        | 画面     | Open/Done/Discarded一覧・完了申告              |
...
stateDiagram-v2
    ...
    S07(ホーム) --> S10(Action_Ticket一覧) : チケット一覧を開く

    %% Recommendationフロー
    S09(Recommendation) --> S09(Recommendation) : 「いいえ（別の方法で）」→ 同Goal内別提案
    S09(Recommendation) --> S09(Recommendation) : 「目標チェンジ」→ Pivot提案
    S09(Recommendation) --> S10(Action_Ticket一覧) : 4択応答でTicket生成
    S10(Action_Ticket一覧) --> S07(ホーム) : ホームへ戻る
